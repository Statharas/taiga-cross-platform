...
void LibraryMenu::remove() const {
  const auto info = QFileInfo{m_path};
  const auto message = tr("Move \"%1\" to trash?").arg(info.fileName());
  if (QMessageBox::question(parentWidget(), tr("Delete"), message) != QMessageBox::Yes) return;

  if (!QFile::moveToTrash(m_path)) {
    QMessageBox::warning(parentWidget(), tr("Delete"),
                         tr("Could not move \"%1\" to trash.").arg(info.fileName()));
  }
}
...
void LibraryMenu::rename() const {
  const auto info = QFileInfo{m_path};
  bool ok = false;
  const auto name = QInputDialog::getText(parentWidget(), tr("Rename"), tr("Name:"),
                                          QLineEdit::Normal, info.fileName(), &ok);
  if (!ok || name.trimmed().isEmpty() || name == info.fileName()) return;

  QDir dir{info.absolutePath()};
  if (!dir.rename(info.fileName(), name)) {
    QMessageBox::warning(parentWidget(), tr("Rename"),
                         tr("Could not rename \"%1\".").arg(info.fileName()));
  }
}
